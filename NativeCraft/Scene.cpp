...
Scene::Scene(ContextParameters* params)
{
	...
	_gBuffer->Bind();
	{
		...
		_tPosition = new Texture(params->width, params->height, GL_RGB32F, GL_RGB, GL_FLOAT, false);
		_tPosition->Bind();
		{
		...
		_tNormal = new Texture(params->width, params->height, GL_RGB, GL_RGB, GL_FLOAT, false);
		_tNormal->Bind();
		{
		...
		_tAlbedo = new Texture(params->width, params->height, GL_RGB, GL_RGB, GL_UNSIGNED_BYTE, false);
		_tAlbedo->Bind();
		{
	...
	_ssaoBuffer->Bind();
	{
		_tSsao = new Texture(params->width, params->height, GL_RED,GL_RGB, GL_FLOAT, false);
	...
	{
		_tSsaoBlur = new Texture(params->width, params->height, GL_RED, GL_RGB, GL_FLOAT, false);
	...
	_sampleTex = new Texture(4, 4, GL_RGB32F,GL_RGB, GL_FLOAT, false);
	_sampleTex->Bind();
	{
		_sampleTex->SetParameter(GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		_sampleTex->SetParameter(GL_TEXTURE_MAG_FILTER, GL_NEAREST);


		_sampleTex->SetData(ssaoKernel.data());
	}
	...
	_noiseTexture = new Texture(4, 4, GL_RGB32F,GL_RGB, GL_FLOAT, false);
...
/*
	...
	*/
	...
	_ssaoBuffer->Bind();
	{
		...
		//_ssao->SetMatrix("projectionInverse", _camera->ProjectionInverse);


		_rb->DrawTextures(4, new Texture * [4]{ _tPosition, _tNormal, _noiseTexture, _sampleTex }, 0, 0, 2560, 1440, _mTransforms, _ssao);
	}
	_ssaoBuffer->Unbind();
	...
	_ssaoLight->SetVector("LightDirection", vec3(1, 1, 1));
